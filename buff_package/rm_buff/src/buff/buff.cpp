...
Buff::Buff() {
    ...
    input_size        = { 640, 640 };
    last_bullet_speed = 0;
    fmt::print( fmt::fg( fmt::color::pale_violet_red ),
                "[BUFF] Buff init model success! Size: {} {}\n",
                input_size.height, input_size.width );

#ifdef SAVE_BUFF_LOG
    LOG( INFO ) << "[BUFF] Buff init model success! Size: " << input_size.height
                << " " << input_size.width;
#endif // SAVE_BUFF_LOG
}
...
bool Buff::chooseFromTargets( vector<Fan> &fans, Fan &target ) {
...
// TODO:修复ROI
    ...
    if ( 0 ) {
        ...
        line( src.img, Point2f( src.img.size().width / 2, 0 ),
              Point2f( src.img.size().width / 2, src.img.size().height ),
              Scalar( 0, 255, 0 ), 1 );
        line( src.img, Point2f( 0, src.img.size().height / 2 ),
              Point2f( src.img.size().width, src.img.size().height / 2 ),
              Scalar( 0, 255, 0 ), 1 );
    ...
    for ( auto object : objects ) {
        ...
        // auto pnp_result = coordsolver.pnp( points_pic, rmat_imu, target_type,
        //                                    SOLVEPNP_ITERATIVE );
        //--------------------------------------------------------------------------//
    ...
    if ( !is_target_exists ) {
        ...
        for ( auto fan : fans ) {
            putText( src.img, fmt::format( "{:.2f}", fan.conf ),
                     fan.apex2d[ 4 ], FONT_HERSHEY_SIMPLEX, 1, { 0, 255, 0 },
                     2 );
            if ( fan.color == 0 )
        ...
        is_last_target_exists = false;
        data                  = { {(float) 0}, {(float) 0}, {(float) 0}, 0, 0, 0, 1 };
        LOG( WARNING ) << "[BUFF] No available target fan or Multiple target "
                          "fan detected!";
        return false;
    }
    ...
    if ( !predictor.CurveFitting( mean_rotate_speed, mean_r_center.norm(),
    ...
    auto r_center_cam =
        coordsolver.worldToCam( target.centerR3d_world, rmat_imu );
    ...
    double dr_full_ms =
        std::chrono::duration<double, std::milli>( time_predict - time_start )
            .count();
    double dr_crop_ms =
        std::chrono::duration<double, std::milli>( time_crop - time_start )
            .count();
    double dr_infer_ms =
        std::chrono::duration<double, std::milli>( time_infer - time_crop )
            .count();
    double dr_predict_ms =
        std::chrono::duration<double, std::milli>( time_predict - time_infer )
            .count();
...
#ifdef SAVE_BUFF_LOG
    LOG( INFO ) << "[BUFF] LATENCY: "
                << "Crop: " << dr_crop_ms << " ms"
                << " Infer: " << dr_infer_ms << " ms"
                << " Predict: " << dr_predict_ms << " ms"
                << " Total: " << dr_full_ms << " ms";
    LOG( INFO ) << "[BUFF] TARGET_INFO: "
                << "Yaw: " << angle[ 0 ] << " Pitch: " << angle[ 1 ]
                << " Dist: " << (float) hit_point_cam.norm()
                << "Is Switched:" << is_switched;
#endif // SAVE_BUFF_LOG
    data = { (float) angle[ 1 ],
             (float) angle[ 0 ],
             (float) hit_point_cam.norm(),
