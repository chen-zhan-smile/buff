...
/**
 * @brief 加载BUFF::CoordSolver参数
 *
 * @param coord_path 参数文件路径
 * @param param_name 参数组名称
 * @return bool 加载是否成功
 */
bool BUFF::CoordSolver::loadParam( string coord_path, string param_name ) {
...
BUFF::PnPInfo BUFF::CoordSolver::pnp( const std::vector<Point2f> &points_pic,
                                      const Eigen::Matrix3d &     rmat_imu,
    ...
    solvePnP( points_world, points_pic, intrinsic, dis_coeff, rvec, tvec, false,
              method );

    PnPInfo result;
    // Pc = R * Pw + T
    Rodrigues( rvec, rmat );      //向量转换成矩阵
    cv2eigen( rmat, rmat_eigen ); // Mat转化成eigen
    cv2eigen( tvec, tvec_eigen ); // Mat转化成eigen
    {
        // clang-format off
 ...
 */
Eigen::Vector2d BUFF::CoordSolver::getAngle( Eigen::Vector3d &xyz_cam,
                                             Eigen::Matrix3d &rmat ) {
    // cout<<xyz_cam<<endl;
    // cout<<endl;
...
/**
 * @brief 重投影
 * @description: 将三维坐标点投影到相机坐标上
 * @param xyz 目标三维坐标
 * @return cv::Point2f 图像坐标系上坐标(x,y)
 */
cv::Point2f BUFF::CoordSolver::reproject( Eigen::Vector3d &xyz ) {
    Eigen::Matrix3d mat_intrinsic;
    cv2eigen( intrinsic, mat_intrinsic );
    //(u,v,1)^T = (1/Z) * K * (X,Y,Z)^T
    auto result =
        ( 1.f / xyz[ 2 ] ) * mat_intrinsic * ( xyz ); //解算前进行单位转换
    return cv::Point2f( result[ 0 ], result[ 1 ] );
}

/**
 * @brief 静态坐标补偿
 * @description: 坐标有误差可以通过静态补偿来缩小
 * @param xyz 待补偿坐标
 * @return Eigen::Vector3d 补偿后坐标
 */
inline Eigen::Vector3d
    BUFF::CoordSolver::staticCoordOffset( Eigen::Vector3d &xyz ) {
    return xyz + xyz_offset;
}

/**
 * @brief 静态角度补偿
 * @description: 角度有误差可以通过静态补偿来缩小
 * @param angle 待补偿角度
 * @return Eigen::Vector2d 补偿后角度
 */
inline Eigen::Vector2d
    BUFF::CoordSolver::staticAngleOffset( Eigen::Vector2d &angle ) {
    return angle + angle_offset;
}

inline double BUFF::CoordSolver::calcYaw( Eigen::Vector3d &xyz ) {
 ...
 */
inline double BUFF::CoordSolver::calcPitch( Eigen::Vector3d &xyz ) {
    return -(
        atan2( xyz[ 1 ], sqrt( xyz[ 0 ] * xyz[ 0 ] + xyz[ 2 ] * xyz[ 2 ] ) ) *
        180 / CV_PI );
    // return (atan2(xyz[1], sqrt(xyz[0] * xyz[0] + xyz[2] * xyz[2])) * 180 /
    // CV_PI);
}
 ...
 * @return Yaw与Pitch
 */
inline Eigen::Vector2d BUFF::CoordSolver::calcYawPitch( Eigen::Vector3d &xyz ) {
 ...
 */
inline double
    BUFF::CoordSolver::dynamicCalcPitchOffset( Eigen::Vector3d &xyz ) {
    // TODO:根据陀螺仪安装位置调整距离求解方式
    //降维，坐标系Y轴以垂直向上为正方向
    auto dist_vertical = xyz[ 2 ];
    auto vertical_tmp  = dist_vertical;
    auto dist_horizonal =
        sqrt( xyz.squaredNorm() - dist_vertical * dist_vertical );
    // auto dist_vertical = xyz[2];
    // auto dist_horizonal = sqrt(xyz.squaredNorm() - dist_vertical *
    // dist_vertical);
 ...
 * **/
Eigen::Vector3d
    BUFF::CoordSolver::camToWorld( const Eigen::Vector3d &point_camera,
    ...
    point_camera_tmp << point_camera[ 0 ], point_camera[ 1 ], point_camera[ 2 ],
        1;
 ...
 * **/
Eigen::Vector3d
    BUFF::CoordSolver::worldToCam( const Eigen::Vector3d &point_world,
                                   const Eigen::Matrix3d &rmat ) {
    ...
    point_camera << point_camera_tmp[ 0 ], point_camera_tmp[ 1 ],
        point_camera_tmp[ 2 ];

    return point_camera;
}

bool BUFF::CoordSolver::setBulletSpeed( double speed ) {
    bullet_speed = speed;
    return true;
}