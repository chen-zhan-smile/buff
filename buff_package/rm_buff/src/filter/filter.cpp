...
bool ParticleFilter::initParam( YAML::Node &config, const string param_name ) {
    ...
    //初始化过程噪声矩阵
    auto read_vector =
        config[ param_name ][ "process_noise" ].as<vector<float>>();
    ...
    matrix_weights =
        Eigen::MatrixXd::Ones( num_particle, 1 ) / float( num_particle );
    is_ready = false;
...
Eigen::VectorXd ParticleFilter::predict() {
    Eigen::VectorXd particles_weighted =
        matrix_particle.transpose() * matrix_weights;
...
bool ParticleFilter::update( Eigen::VectorXd measure ) {
    Eigen::MatrixXd gaussian =
        Eigen::MatrixXd::Zero( num_particle, vector_len );
    Eigen::MatrixXd mat_measure =
        measure.replicate( 1, num_particle ).transpose();
    auto err = ( ( measure - ( matrix_particle.transpose() * matrix_weights ) )
                     .norm() );
    ...
    if ( is_ready ) {
        ...
        for ( int i = 0; i < matrix_particle.cols(); i++ ) {
            ...
            Eigen::MatrixXd weights_dist =
                ( matrix_particle.col( i ) - mat_measure.col( i ) )
                    .rowwise()
                    .squaredNorm();
            Eigen::MatrixXd tmp = ( ( -( weights_dist / pow( sigma, 2 ) ) /
                                      matrix_particle.cols() )
                                        .array()
                                        .exp() /
                                    ( sqrt( CV_2PI ) * sigma ) )
                                      .array();
        ...
        double n_eff =
            1.0 / ( matrix_weights.transpose() * matrix_weights ).value();
        // TODO:有效粒子数少于一定值时进行重采样,该值需在实际调试过程中修改
        // if (n_eff < 0.5 * num_particle)
        if ( err > observe_noise_cov( 0, 0 ) ||
             ( n_eff < 0.5 * num_particle ) ) {
...
bool ParticleFilter::resample() {
    ...
    //重采样采用低方差采样,复杂度为O(N),较轮盘法的O(NlogN)更小,实现可参考<Probablistic
    // Robotics>
    std::random_device    rd;
    default_random_engine e( rd() );
    // clang-format off
    std::uniform_real_distribution<> random{ 0.0,double( 1.0 ) / num_particle };
    // clang-format on
    ...
    for ( int m = 0; m < num_particle; m++ ) {
        auto u = r + m * ( double( 1.0 ) / num_particle );
        // 当 u > c 不进行采样
        while ( u > c ) {
            i++;
